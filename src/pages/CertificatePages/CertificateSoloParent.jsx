...
function CertificateSoloParent({ setActiveCertificate }) {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/soloParent`);
  ...
  const handleDropdown = async (action, requestId) => {
    switch (action) {
      case 'generate-Certificate':
        navigate(`/barangay-certificate-for-solo-parent/${requestId}`);
        break;
      case 'View-Details':
        navigate(`/view-details-solo-parent/${requestId}`);
        break;
      case 'Completed':
          try {
              ...
              await axios.delete(`${API_BASE_URL}/api/soloParent/${requestId}`);
  ...
  const filteredRequests = requests.filter((request) =>
    request.parent.toLowerCase().includes(searchTerm.toLowerCase()) // Filter by fullName
  );
      ...
      <div className="text-sm">
        <div className="flex justify-between items-center h-14 w-full border-b-2 border-b-gray-400">
          ...
          <div className='flex justify-center items-center gap-x-2 text-sm'>
            ...
            /
            <p className='text-gray-400 cursor-pointer'>Solo Parent</p>
          </div>
        </div>
        ...
        {filteredRequests.length === 0 ? (
          <div className="text-center py-5 text-gray-500">No one to issue for Certificate For Solo Parent</div>
        ) : (
          <div>
            <table className="w-full border-collapse">
              <thead className="bg-Green">
                <tr>
                  ...
                  <th className="px-6 py-3 text-xs font-medium text-White uppercase tracking-wider border-l border-r border-gray-300">Parent</th>
                  <th className="px-6 py-3 text-xs font-medium text-White uppercase tracking-wider border-l border-r border-gray-300">Child</th>
                  <th className="px-6 py-3 text-xs font-medium text-White uppercase tracking-wider border-l border-r border-gray-300">Date of Birth</th>
              ...
              <tbody className="text-center divide-y divide-gray-300">
                {currentFilteredRequests.map((request, index) => (
                  <tr key={request._id} className="hover:bg-white">
                    ...
                    <td className="py-4 text-sm">{request.parent}</td> 
                    <td className="py-3 text-sm">{request.child}</td>
                    <td className="py-3 text-sm">{formatDate(request.dateOfBirth)}</td> 
