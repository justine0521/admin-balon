...
function DeathCertificate() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        // Replace with the correct certId for Death Certificates
        const response = await axios.get('http://localhost:5000/api/certificates?certId=6'); // Use certId for Death Certificate
    ...
    <div className="w-4/5 h-screen mt-14 left-56 p-7 absolute">
      <div className="bg-White">
        <div className="flex justify-between items-center h-16 bg-White px-5 w-full border border-b-gray-300">
          <p className='text-xl'>Certificate of Death Issuance</p>
          <div className='flex justify-center items-center gap-x-2'>
            ...
            <NavLink to={'/certificate-of-death'}>
              {({ isActive }) => (
                <p className={`${isActive ? 'text-gray-300' : 'text-Green'}`}>Certificate of Death</p>
              )}
            </NavLink>
          </div>
        </div>
