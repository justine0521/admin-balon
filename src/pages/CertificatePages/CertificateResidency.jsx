...
function CertificateResidency() {
  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/certificates?certId=2'); // Use certId for Certificate of Residency
            ...
            <tbody className="text-center bg-white divide-y divide-gray-300">
              {requests.slice(startIndex, endIndex).map((request) => (
                <tr key={request._id} className="hover:bg-gray-100">
                  ...
                  <td>
                    <NavLink to={`/generate-residency-certificate/${request._id}`}>
