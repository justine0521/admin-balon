...
function CertificateResidency() {
  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/certificates?certId=2`); // Use certId for Certificate of Residency
            ...
            <tbody className="text-center bg-white divide-y divide-gray-300">
              {requests.slice(startIndex, endIndex).map((request) => (
                <tr key={request._id} className="hover:bg-gray-100">
                  ...
                  <td>
                    <NavLink to={`/barangay-certificate-of-residency/${request._id}`}>
