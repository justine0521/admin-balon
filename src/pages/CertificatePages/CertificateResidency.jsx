...
function CertificateResidency({ setActiveCertificate }) {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/residency`);
  ...
  const handleDropdown = async (action, requestId) => {
    switch (action) {
      case 'generate-Certificate':
        navigate(`/barangay-certificate-of-residency/${requestId}`);
        break;
      case 'View-Details':
        navigate(`/view-details-residency/${requestId}`);
        break;
      case 'Completed':
          try {
              ...
              await axios.delete(`${API_BASE_URL}/api/residency/${requestId}`);
    ...
    <div>
      <div className="text-sm">
        <div className="flex justify-between items-center h-14 w-full border-b-2 border-gray-400">
          <p className='text-xl'>Certificate of Residency Issuance</p>
          ...
          <div className='flex justify-center items-center gap-x-2 text-sm'>
            ...
            /
            <p className='text-gray-400 cursor-pointer'>Certificate of Residency</p>
          </div>
        </div>
        ...
        {filteredRequests.length === 0 ? (
          <p className="text-center py-5 text-gray-500">No one to issue Certificate of Residency</p>
        ) : (
          <div>
            <table className="w-full border-collapse">
              <thead className="bg-Green">
                ...
                <th className="px-6 py-3 text-xs font-medium text-White uppercase tracking-wider border-l border-r border-gray-300"></th>
                  ...
                  <th className="px-6 py-3 text-xs font-medium text-White uppercase tracking-wider border-l border-r border-gray-300">Taon Tumira ng Balon</th>
              ...
              <tbody className="text-center divide-y divide-gray-300">
                {currentFilteredRequests.map((request, index) => (
                  <tr key={request._id} className="hover:bg-white">
                    ...
                    <td className="py-3 text-sm">{formatDate(request.taon)}</td>
