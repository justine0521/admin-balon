...
function CommonLawViewDetails() {
  ...
  useEffect(() => {
    const fetchResidentDetails = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/commonLaw?id=${id}`);
  ...
  const handleBack = () => {
    localStorage.setItem("activeSection", "CommonLaw");
    navigate('/CommonLaw');
};
      ...
      <div className="bg-white p-5 flex flex-col md:flex-row items-start gap-8 h-[600px]">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <p className="text-gray-700"><span className="font-semibold">Pangalan ng Lalaki:</span> {resident.male}</p>
            <p className="text-gray-700"><span className="font-semibold">Pangalan ng Babae:</span> {resident.female}</p>
            <p className="text-gray-700"><span className="font-semibold">Tirahan:</span> {resident.tirahan}</p>
            <p className="text-gray-700"><span className="font-semibold">Ilang Taon/Buwan nang Nagsasama: </span>{resident.yearTogether}</p>
...
export default CommonLawViewDetails;
