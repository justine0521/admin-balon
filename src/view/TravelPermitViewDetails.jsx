...
function TravelPermitViewDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchResidentDetails = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/travelPermit?id=${id}`);
  ...
  const handleBack = () => {
    localStorage.setItem("activeSection", "TravelPermit");
    navigate('/TravelPermit');
};
      ...
      <div className="bg-white p-5 flex flex-col md:flex-row items-start gap-8 h-[600px]">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <p className="text-gray-700"><span className="font-semibold">Pangalan ng May-ari:</span> {resident.owner}</p>
            <p className="text-gray-700"><span className="font-semibold">Address:</span> {resident.address}</p>
            <p className="text-gray-700"><span className="font-semibold">Uri ng Sasakyan:</span> {resident.typeOfCar}</p>
            <p className="text-gray-700"><span className="font-semibold">Plate Number: </span>{resident.plateNumber}</p>
            <p className="text-gray-700"><span className="font-semibold">Anong Scrap ang Ita-Travel: </span>{resident.scrap}</p>
            <p className="text-gray-700"><span className="font-semibold">Pangalan ng Driver: </span>{resident.driver}</p>
            <p className="text-gray-700"><span className="font-semibold">Driver's License: </span>{resident.driverLicense}</p>
            <p className="text-gray-700"><span className="font-semibold">Kailan Ita-Travel: </span>{formatDate(resident.whenToTravel)}</p>
            <p className="text-gray-700"><span className="font-semibold">Saan Dadalhin: </span>{resident.whereToTravel}</p>
...
export default TravelPermitViewDetails;
