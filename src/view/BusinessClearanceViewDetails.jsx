...
function BusinessClearanceViewDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchResidentDetails = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/businessClearance?id=${id}`);
  ...
  const handleBack = () => {
    localStorage.setItem("activeSection", "BusinessClearance");
    navigate('/BusinessClearance');
};
      ...
      <div className="bg-white p-5 flex flex-col md:flex-row items-start gap-8 h-[600px]">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <p className="text-gray-700"><span className="font-semibold">Name of Owner:</span> {resident.owner}</p>
            <p className="text-gray-700"><span className="font-semibold">Name of Business:</span> {resident.business}</p>
            <p className="text-gray-700"><span className="font-semibold">Nature of Business:</span> {resident.natureOfBusiness}</p>
            <p className="text-gray-700"><span className="font-semibold">Address of Business: </span>{resident.address}</p>
...
export default BusinessClearanceViewDetails;
