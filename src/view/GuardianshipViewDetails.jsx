...
function GuardianshipViewDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchResidentDetails = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/guardianship?id=${id}`);
      ...
      <div className="bg-white p-5 flex flex-col md:flex-row items-start gap-8 h-[600px]">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <p className="text-gray-700"><span className="font-semibold">Name of Guardian:</span> {resident.guardian}</p>
            <p className="text-gray-700"><span className="font-semibold">Address:</span> {resident.address}</p>
            <p className="text-gray-700"><span className="font-semibold">Name of Child :</span> {resident.child}</p>
            <p className="text-gray-700"><span className="font-semibold">Birthday of Child: </span>{formatDate(resident.birthdayOfChild)}</p>
            <p className="text-gray-700"><span className="font-semibold">Place of Birth of Child:</span> {resident.placeOfBirth}</p>
...
export default GuardianshipViewDetails;
