...
function SoloParentViewDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchResidentDetails = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/soloParent?id=${id}`);
  ...
  const handleBack = () => {
    localStorage.setItem("activeSection", "CertificateSoloParent");
    navigate('/CertificateSoloParent');
};
      ...
      <div className="bg-white p-5 flex flex-col md:flex-row items-start gap-8 h-[600px]">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            ...
            <p className="text-gray-700"><span className="font-semibold">Name of Parent :</span> {resident.parent}</p>
            ...
            <p className="text-gray-700"><span className="font-semibold">Birthday of Child: </span>{formatDate(resident.dateOfBirth)}</p>
...
export default SoloParentViewDetails;
